...
@author: Ent00002
@modifier: LGuo (l.guo@reading.ac.uk)
"""
#delayed runs
#import time
#time.sleep(500)
...
years		= np.arange(2015,1978,-1)	# fill in the years backward
...
#LGuo: read in mask files of China regions
cn_mask = Dataset('/home/users/lguo/CSSP/WAM2layersPython-master/global_mask_on_1.5_cn1.nc', mode = 'r').variables['msk_china'][latnrs,lonnrs]
cn_mask = cn_mask.astype(np.float64) # cn_mask is in type of np.float32, while lsm is in type of np.float.64
...
interdata_folder = r'/home/users/lguo/CSSP/WAM2layersPython-master/interdata'	#must be an existing folder, existence is not checked
#END OF INPUT

#%% Datapaths (FILL THIS IN)

# Check if interdata folder exists:
# assert statement tests a condiction and triggers an error if the condition is false.
...
def get_Sa_track_backward_TIME(latitude,longitude,count_time,divt,timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,
    W_top,W_down,Sa_track_top_last,Sa_track_down_last,Sa_time_top_last,Sa_time_down_last,Sa_dist_top_last,Sa_dist_down_last,L_N_gridcell,L_S_gridcell,L_EW_gridcell):
    ...
    Sa_dist_down[-1,:,:]	= Sa_dist_down_last
    Sa_dist_top[-1,:,:]		= Sa_dist_top_last
    ...
    # Sa calculation backward in time (96 ~ 0)
    for t in np.arange(np.int(count_time*divt),0,-1):
        # down: define values of total moisture
        Sa_E_down[0,:,:-1] = W_down[t,:,1:]	# Atmospheric storage of the cell to the east [m3]
        # to make dependent on isglobal but for now kept to avoid division by zero errors      
        ...
        #############################################################
        # timetracking start
	# GL: following van der Ent's thesis (2.10), need conformation
        ...
        # time increase, ti = 900 seconds (0.25 hr)
        ti = timestep/divt

        # down: define values of timeed moisture of neighbouring grid cells (1,107,240)
	Sa_time_E_down[0,:,:-1] = Sa_time_down[t,:,1:]		# Atmospheric timeed storage of the cell to the east [units: s]
        if isglobal == 1:
	...
	Sa_time_E_top[0,:,:-1] = Sa_time_top[t,:,1:]	# Atmospheric storage of the cell to the east [units: s]
        if isglobal == 1:
        ...
        # down: add precipitation and substract evaporation. LG: calculate the forth and fifth term of the numerator of Eq. 2.10
	...
	+ Fa_E_down_WE[t-1,1:-1,:] * (xi[0,1:-1,:] + Sa_dist_E_down[0,1:-1,:]) * (Sa_track_E_down[0,1:-1,:] / Sa_E_down[0,1:-1,:]) 
        - Fa_E_down_EW[t-1,1:-1,:] * (               Sa_dist_down[t,1:-1,:])   * (Sa_track_down[t,1:-1,:]   / W_down[t,1:-1,:]) 
        - Fa_W_down_WE[t-1,1:-1,:] * (               Sa_dist_down[t,1:-1,:])   * (Sa_track_down[t,1:-1,:]   / W_down[t,1:-1,:]) 
        + Fa_W_down_EW[t-1,1:-1,:] * (xi[0,1:-1,:] + Sa_dist_W_down[0,1:-1,:]) * (Sa_track_W_down[0,1:-1,:] / Sa_W_down[0,1:-1,:]) 
        + Fa_N_down_SN[t-1,1:-1,:] * (yi           + Sa_dist_N_down[0,1:-1,:]) * (Sa_track_N_down[0,1:-1,:] / Sa_N_down[0,1:-1,:]) 
        - Fa_N_down_NS[t-1,1:-1,:] * (               Sa_dist_down[t,1:-1,:])   * (Sa_track_down[t,1:-1,:]   / W_down[t,1:-1,:]) 
        - Fa_S_down_SN[t-1,1:-1,:] * (               Sa_dist_down[t,1:-1,:])   * (Sa_track_down[t,1:-1,:]   / W_down[t,1:-1,:]) 
        + Fa_S_down_NS[t-1,1:-1,:] * (yi           + Sa_dist_S_down[0,1:-1,:]) * (Sa_track_S_down[0,1:-1,:] / Sa_S_down[0,1:-1,:]) 
        - Fa_downward[t-1,1:-1,:]  * (               Sa_dist_down[t,1:-1,:])   * (Sa_track_down[t,1:-1,:]   / W_down[t,1:-1,:]) 
        + Fa_upward[t-1,1:-1,:]    * (               Sa_dist_top[t,1:-1,:])    * (Sa_track_top[t,1:-1,:]    / W_top[t,1:-1,:])
        ) / Sa_track_after_Fa_down[0,1:-1,:])
	...
	Sa_dist_after_Fa_top[0,1:-1,:] = ((Sa_track_top[t,1:-1,:]  * (               Sa_dist_top[t,1:-1,:]) 
        + Fa_E_top_WE[t-1,1:-1,:] * (xi[0,1:-1,:] + Sa_dist_E_top[0,1:-1,:]) * (Sa_track_E_top[0,1:-1,:] / Sa_E_top[0,1:-1,:]) 
        - Fa_E_top_EW[t-1,1:-1,:] * (               Sa_dist_top[t,1:-1,:])   * (Sa_track_top[t,1:-1,:]   / W_top[t,1:-1,:])  
        - Fa_W_top_WE[t-1,1:-1,:] * (               Sa_dist_top[t,1:-1,:])   * (Sa_track_top[t,1:-1,:]   / W_top[t,1:-1,:]) 
        + Fa_W_top_EW[t-1,1:-1,:] * (xi[0,1:-1,:] + Sa_dist_W_top[0,1:-1,:]) * (Sa_track_W_top[0,1:-1,:] / Sa_W_top[0,1:-1,:])
        + Fa_N_top_SN[t-1,1:-1,:] * (yi           + Sa_dist_N_top[0,1:-1,:]) * (Sa_track_N_top[0,1:-1,:] / Sa_N_top[0,1:-1,:]) 
        - Fa_N_top_NS[t-1,1:-1,:] * (               Sa_dist_top[t,1:-1,:])   * (Sa_track_top[t,1:-1,:]   / W_top[t,1:-1,:])
        - Fa_S_top_SN[t-1,1:-1,:] * (               Sa_dist_top[t,1:-1,:])   * (Sa_track_top[t,1:-1,:]   / W_top[t,1:-1,:]) 
        + Fa_S_top_NS[t-1,1:-1,:] * (yi           + Sa_dist_S_top[0,1:-1,:]) * (Sa_track_S_top[0,1:-1,:] / Sa_S_top[0,1:-1,:]) 
        + Fa_downward[t-1,1:-1,:] * (               Sa_dist_down[t,1:-1,:])  * (Sa_track_down[t,1:-1,:]  / W_down[t,1:-1,:]) 
        - Fa_upward[t-1,1:-1,:]   * (               Sa_dist_top[t,1:-1,:])   * (Sa_track_top[t,1:-1,:]   / W_top[t,1:-1,:]) 
        ) / Sa_track_after_Fa_top[0,1:-1,:])
	...
	#	+ P_region[t-1,1:-1,:] * di[0,1:-1,:]/2. * (W_down[t,1:-1,:] / W[t,1:-1,:]) 
		- E[t-1,1:-1,:] * (               Sa_dist_down[t,1:-1,:]) * (Sa_track_down[t,1:-1,:] / W_down[t,1:-1,:]) 
        ) / Sa_track_after_Fa_P_E_down[0,1:-1,:])
	...
	#	+ P_region[t-1,1:-1,:] * di[0,1:-1,:]/2 * (W_top[t,1:-1,:] / W[t,1:-1,:])
        ) / Sa_track_after_Fa_P_E_top[0,1:-1,:])
        ...
        - down_to_top[t-1,:,:] * Sa_dist_after_Fa_P_E_down
        + top_to_down[t-1,:,:] * Sa_dist_after_Fa_P_E_top
        ) / Sa_track_after_all_down)
...
def create_empty_array(count_time,divt,latitude,longitude,yearpart,years):
    Sa_time_top   = np.zeros((np.int(count_time*divt)+1,len(latitude),len(longitude)))		# shape: (97,107,240)
...
for i in range(len(years)):
	...
	Fa_E_top  = loading_FS['Fa_E_top']			# For these four variables, (96,107,240), units: m3
        Fa_N_top  = loading_FS['Fa_N_top']			# temporal information of fluxes has been removed
        ...
        if timetracking == 0:
            Sa_track_top,Sa_track_down,north_loss,south_loss,down_to_top,top_to_down,water_lost = get_Sa_track_backward(latitude,longitude,count_time,divt,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last)
        elif timetracking == 1:
            ...
            Sa_dist_top			= loading_STT['Sa_dist_top']			# [m]
            Sa_dist_down		= loading_STT['Sa_dist_down']            
            Sa_dist_top_last_scheef	= Sa_dist_top[0,:,:]
            Sa_dist_down_last_scheef	= Sa_dist_down[0,:,:]
            Sa_dist_top_last		= np.reshape(Sa_dist_top_last_scheef, (1,len(latitude),len(longitude)))
            Sa_dist_down_last		= np.reshape(Sa_dist_down_last_scheef, (1,len(latitude),len(longitude)))
            
            Sa_dist_top,Sa_dist_down,Sa_time_top,Sa_time_down,Sa_track_top,Sa_track_down,north_loss,south_loss,down_to_top,top_to_down,water_lost = get_Sa_track_backward_TIME(latitude,longitude,count_time,divt,timestep,Kvf,Region,Fa_E_top,Fa_N_top,Fa_E_down,Fa_N_down,Fa_Vert,E,P,W_top,W_down,Sa_track_top_last,Sa_track_down_last,Sa_time_top_last,Sa_time_down_last,Sa_dist_top_last,Sa_dist_down_last,L_N_gridcell,L_S_gridcell,L_EW_gridcell)
        ...
        if timetracking == 1:
            sio.savemat(datapath[4], {'Sa_time_top':Sa_time_top,'Sa_time_down':Sa_time_down,'Sa_dist_top':Sa_dist_top,'Sa_dist_down':Sa_dist_down},do_compression=True)
